 ...
 * @author geek alaa
 */
public class AffichageArticleParTags implements Initializable{
...
public void AffichParTag(int id){
      GestionArticles gst = new GestionArticles();
       ...
       for (int i = 0; i < listarticles.size()  ; i++) {
            try {
                ...
                for(int j=0;j<listarticles.get(i).getListags().size(); j++){
                if(listarticles.get(i).getListags().get(j).getId() == id){
